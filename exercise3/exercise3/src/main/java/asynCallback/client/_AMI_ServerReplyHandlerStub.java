	...
	implements asynCallback.client.AMI_ServerReplyHandler
{
	...
	public void updateStock_excep(org.omg.Messaging.ExceptionHolder excep_holder)
	{
		while(true)
		{
			if(! this._is_local())
			{
				...
				try
				{
					_os = _request( "updateStock_excep", true);
		...
		{
			org.omg.CORBA.portable.ServantObject _so = _servant_preinvoke( "updateStock_excep", _opsClass );
			if( _so == null )
				continue;
			AMI_ServerReplyHandlerOperations _localServant = (AMI_ServerReplyHandlerOperations)_so.servant;
			try
			{
				_localServant.updateStock_excep(excep_holder);
	...
	public void updateStock(asynCallback.client.Stock ami_return_val)
	{
		...
		else
		{
			...
			try
			{
				_localServant.updateStock(ami_return_val);
				if ( _so instanceof org.omg.CORBA.portable.ServantObjectExt) 
					((org.omg.CORBA.portable.ServantObjectExt)_so).normalCompletion();
				return;
			}
